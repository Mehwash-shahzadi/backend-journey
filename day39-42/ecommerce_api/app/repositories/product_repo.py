...
def create_product_repo(session: AsyncSession):
    """Factory function  returns a ready-to-use ProductRepository instance."""
    return ProductRepository(session)


product_repo = type("ProductRepo", (), {
    "get_by_id": lambda session, pid: ProductRepository(session).get_by_id(pid),
    "get_all_with_categories": lambda session: ProductRepository(session).get_all_with_categories(),
})